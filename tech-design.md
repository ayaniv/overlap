...
makes the change safe to merge before `overlap-api` is deployed.
...
6. **CORS origins to set at deploy time (outside this repo).** The site's real
   ...
   `.env.example` lists only `https://overlap.vercel.app`. Its deployed
...
- Both requests use `AbortSignal.any([AbortSignal.timeout(SHORT_LINK_REQUEST_TIMEOUT_MS), signal])`,
  leaving out `signal` when it isn't given. A `TimeoutError` DOMException maps to
...
- `persistConfig` writes `history.replaceState(null, '', \`/${HASH_PREFIX}${encoded}\`)`,
  an **absolute root path**. After a short link resolves, or fails, the address bar
  reads `/#c=…`. The session stays self-contained: reloading doesn't hit the API again,
  and the hash fallback share URL (`location.href`) is a clean hash link, not
  `/abc123#c=…`. (Before the rewrite rule the app only ever ran at `/`, so this
  changes nothing for today's URLs.)
...
- `public/privacy.html`: add a "Short links" paragraph. It says that when you share,
  the app stores a snapshot of your city list and working hours (never meetings, never
  any Google data) on overlap's own server so the link can be short, and that anyone
  with the link can open it. It also covers what the old link format still does. Then
  scope the "Nothing is collected or stored" and "There is no server" sentences to
  match. The Google-data sections stay **true as written**, because decision 3 keeps
  them true. Don't touch them beyond fixing a sentence that becomes literally false.
...
**QA Spec:** `src/shortLinks/shortLinkApi.test.ts`, `src/shortLinks/overlapApiConfig.test.ts`, `src/shortLinks/useShortLinkPrefetch.test.tsx`, `src/App.test.tsx` (the two new `App — … short link …` describe blocks), `src/vercelConfig.test.ts` and `src/shortLinks/shortLinkApi.live.test.ts` (opt-in)
...
Red at commit time: 8 files fail (4 of them at import time) / 17 tests fail, for the expected reasons (missing
modules, missing `rewrites`, missing loading test id, analytics payloads). The
regression guards (hash beats path, root path, env-unset path, no persist while
resolving) pass today and must stay green. The live file fails only on its import
