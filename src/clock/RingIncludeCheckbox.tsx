...
// disabledReason is required whenever disabled is true (shown as a hover
// tooltip explaining why) — a disabled control with no explanation is
// exactly the silent-failure problem this prop exists to avoid. Modeled as a
// discriminated union, not two independent optional fields, so that
...
export function RingIncludeCheckbox(props: RingIncludeCheckboxProps) {
  const { location, dotPosition, checked, disabled, onToggle } = props;
  const disabledReason = props.disabled ? props.disabledReason : undefined;
  return (
    <label
